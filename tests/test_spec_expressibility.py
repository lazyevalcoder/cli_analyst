...
class TestCountShare:
    ...
    def test_bare_share_with_condition(self):
        spec = {"name": "Express Ship Adoption", "agg": "share", "condition": COND, "compare": "pct_change", "unit": "ratio"}
...
class TestScalarAggOverDerived:
    ...
    def test_mean_of_days_between_derived(self):
        spec = {
            "name": "Order-to-Ship Cycle",
            "agg": "mean",
            "prep": [{"op": "derive.days_between", "start": "engage", "end": "ship", "as": "cycle_days"}],
            "value_column": "cycle_days",
            "compare": "pct_change",
            "unit": "days",
        }
        assert builder._validate_spec(spec, set(_df().columns))[0]
    ...
    def test_group_step_null_group_by_whole_frame(self):
        ...
        spec = {
            "name": "Whole Frame Cycle",
            "agg": None,
            "prep": [{"op": "derive.days_between", "start": "engage", "end": "ship", "as": "cycle_days"}],
            "steps": [{"op": "group", "group_by": None, "inner_agg": "mean", "value": "cycle_days", "outer_agg": "mean"}],
            "compare": "level",
            "unit": "days",
        }
        assert builder._validate_spec(spec, set(_df().columns))[0]
...
class TestMedian:
    def test_median_agg(self):
        spec = {"name": "Median Value", "agg": "median", "value_column": "value", "compare": "pct_change", "unit": "currency"}
        assert builder._validate_spec(spec, set(_df().columns))[0]
        out = _run([spec])[spec["name"]]
    ...
    def test_median_in_group_step(self):
        spec = {
            "name": "Median Per Agent",
            "agg": None,
            "steps": [{"op": "group", "group_by": "agent", "inner_agg": "median", "value": "value", "outer_agg": "mean"}],
            "compare": "pct_change",
            "unit": "currency",
        }
        assert builder._validate_spec(spec, set(_df().columns))[0]
...
class TestSimpleAggs:
    ...
    def test_agg_with_condition(self, agg, expected):
        spec = {"name": "Metric", "agg": agg, "value_column": "value", "condition": COND, "compare": "pct_change", "unit": "ratio"}
        assert builder._validate_spec(spec, set(_df().columns))[0]
        out = _run([spec])[spec["name"]]
...
class TestPrecheckDoesNotBlockExpressibleShapes:
    def test_count_share_measurement_flows_to_llm(self):
        ...
        reason, prim = builder._precheck_measurement(
            "Percentage change in share of orders using 'Express Air' compared to the prior period", period, df=_df()
        )
    ...
    def test_derived_mean_measurement_flows_to_llm(self):
        period = _period()
        reason, prim = builder._precheck_measurement(
            "Percentage change in mean(difference between Ship Date and Order Date) compared to the prior period", period, df=_df()
        )
        assert not reason, reason
    ...
    def test_delta_still_rejected_when_prior_is_empty(self):
        ...
        reason, prim = builder._precheck_measurement("Percentage change in share of orders using 'Express Air'", period, df=_df())
