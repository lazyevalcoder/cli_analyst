...
def _all_aggs_specs():
    return [
        {"name": "CountLvl", "agg": "count", "value_column": None, "condition": None,
         "numerator": None, "denominator": None, "group_by": None, "k": None,
         "compare": "level", "unit": "count"},
        {"name": "CountPct", "agg": "count", "value_column": None, "condition": None,
         "numerator": None, "denominator": None, "group_by": None, "k": None,
         "compare": "pct_change", "unit": "ratio"},
        {"name": "CountPP", "agg": "count", "value_column": None, "condition": None,
         "numerator": None, "denominator": None, "group_by": None, "k": None,
         "compare": "pp_change", "unit": "pp"},
        {"name": "CountRate", "agg": "count", "value_column": None, "condition": None,
         "numerator": None, "denominator": None, "group_by": None, "k": None,
         "compare": "rate_ratio", "unit": "ratio"},
        {"name": "SumLvl", "agg": "sum", "value_column": "value", "condition": None,
         "numerator": None, "denominator": None, "group_by": None, "k": None,
         "compare": "level", "unit": "currency"},
        {"name": "MeanLvl", "agg": "mean", "value_column": "value", "condition": None,
         "numerator": None, "denominator": None, "group_by": None, "k": None,
         "compare": "level", "unit": "currency"},
        {"name": "StdLvl", "agg": "std", "value_column": "value", "condition": None,
         "numerator": None, "denominator": None, "group_by": None, "k": None,
         "compare": "level", "unit": "currency"},
        {"name": "RatioLvl", "agg": "ratio",
         "numerator": {"agg": "sum", "value_column": "value", "condition": None},
         "denominator": {"agg": "count", "value_column": None, "condition": None},
         "value_column": None, "condition": None, "group_by": None, "k": None,
         "compare": "level", "unit": "currency"},
        {"name": "ShareLvl", "agg": "share",
         "numerator": {"agg": "sum", "value_column": "value", "condition": None},
         "denominator": {"agg": "sum", "value_column": "value", "condition": None},
         "value_column": None, "condition": None, "group_by": None, "k": None,
         "compare": "level", "unit": "ratio"},
        {"name": "TopK", "agg": "topk_share", "value_column": "value", "condition": None,
         "numerator": None, "denominator": None, "group_by": "date", "k": 1,
         "compare": "level", "unit": "ratio"},
    ]
...
class TestSpecs:
    ...
    def test_validate_topk_requires_k(self):
        spec = {"name": "X", "agg": "topk_share", "value_column": "value", "group_by": "date",
                "k": None, "compare": "level", "unit": "ratio"}
...
class TestTemplate:
    ...
    def test_template_with_condition(self):
        specs = [{
            "name": "Won Count", "agg": "count", "value_column": None,
            "condition": "df['value'] > 2", "compare": "level", "unit": "count",
        }]
    ...
    def test_pct_change_zero_prior_null_reason(self):
        spec = {"name": "Growth", "agg": "count", "value_column": None, "condition": None,
                "numerator": None, "denominator": None, "group_by": None, "k": None,
...
class TestPeriodBounds:
    ...
    def test_valid_bounds(self):
        ...
        assert not builder._valid_bounds({"current_start": None, "current_end": None,
                                          "prior_start": None, "prior_end": None})
        assert not builder._valid_bounds({"current_start": "2026-08-31", "current_end": "2026-08-01",
                                          "prior_start": "2026-07-01", "prior_end": "2026-07-31"})
...
class TestMetricBrief:
    ...
    def test_brief_with_values(self):
        values = {
            "Growth": {"value": 0.5, "unit": "ratio", "period": "Aug-2026",
                       "status": "computed", "verified": False}
        }
...
class TestDataLimitedPreFilter:
    ...
    def test_expressible_measurements_not_filtered(self):
        for text in ("current count vs prior count", "mean win rate across agents",
                     "top 20% of products by revenue", "new account acquisition rate"):
...
class TestComputePriorityValues:
    ...
    def test_compute_resumes_existing(self, monkeypatch):
        ...
        existing = {"Deal Count": {
            "value": 42, "unit": "count", "status": "computed", "verified": False,
        }}
        calls = iter([
            _period(),
            [_specs()[0]],
    ...
    def test_compute_script_failure_is_error(self, monkeypatch):
        ...
        monkeypatch.setattr(builder.llm, "ask_json", lambda *a, **k: next(calls))
        monkeypatch.setattr(builder.sandbox, "execute_code",
                            lambda *a, **k: (False, "boom: simulated sandbox failure"))
        result = builder.compute_priority_values(_sample_pri(), _sample_df(), "schema")
        values = result["priorities"]["Growth"]["values"]
...
def _group_spec(name, inner, outer, value="value", compare="level", unit="ratio", group_by="agent"):
    return {
        "name": name,
        "steps": [{"op": "group", "group_by": group_by, "inner_agg": inner,
                   "value": value, "outer_agg": outer}],
        "compare": compare, "unit": unit,
    }
...
class TestOperatorDSL:
    ...
    def test_group_count_uses_size_not_column(self):
        ...
        spec = dict(_group_spec("CountOnly", "count", "sum"), steps=[
            {"op": "group", "group_by": "agent", "inner_agg": "count",
             "value": None, "outer_agg": "sum"}])
        parsed = _run([spec])
        assert abs(parsed["CountOnly"]["value"] - 4.0) < 1e-9  # 2 + 2

    def test_validate_group_bad_inner(self):
        spec = dict(_group_spec("X", "count", "mean"), steps=[
            {"op": "group", "group_by": "agent", "inner_agg": "bogus",
             "value": None, "outer_agg": "mean"}])
        ok, msg = builder._validate_spec(spec, {"agent", "value"})
...
class TestDerive:
    def test_derive_days_between_group(self):
        spec = {
            ...
            "steps": [{"op": "group", "group_by": "date", "inner_agg": "mean",
    ...
    def test_derive_arithmetic_group(self):
        spec = {
            ...
            "steps": [{"op": "group", "group_by": "date", "inner_agg": "sum",
                       "value": "double", "outer_agg": "mean"}],
            "compare": "level", "unit": "count",
        }
        parsed = _run([spec], df=_derive_df())
    ...
    def test_validate_derive_collides_with_column(self):
        spec = {
            ...
            "steps": [{"op": "group", "group_by": "date", "inner_agg": "count",
                       "value": None, "outer_agg": "mean"}],
            "compare": "level", "unit": "count",
        }
        ok, msg = builder._validate_spec(spec, {"date"})
...
class TestWindowAndPercentile:
    def test_count_distinct_top_level(self):
        spec = {"name": "Agents", "agg": "count_distinct", "value_column": "agent",
                "compare": "level", "unit": "count"}
        parsed = _run([spec])
    ...
    def test_new_first_time_in_current_period(self):
        spec = {"name": "NewAgents", "steps": [{"op": "new", "value_column": "agent"}],
                "compare": "level", "unit": "count"}
    ...
    def test_new_degenerates_to_distinct_without_history(self):
        spec = {"name": "NewAll", "steps": [{"op": "new", "value_column": "agent"}],
                "compare": "level", "unit": "count"}
    ...
    def test_new_prior_value(self):
        spec = {"name": "NewAgentsPct", "steps": [{"op": "new", "value_column": "agent"}],
                "compare": "pct_change", "unit": "count"}
        parsed = _run([spec], df=_new_df())
    ...
    def test_fractional_topk_share(self):
        spec = {"name": "TopHalf", "agg": "topk_share", "value_column": "value",
                "group_by": "agent", "k": 0.5, "compare": "level", "unit": "ratio"}
        parsed = _run([spec])
        # top 50% of 2 agents = top 1 -> B (70/100)
        assert abs(parsed["TopHalf"]["value"] - 0.7) < 1e-9

    def test_validate_fractional_topk_bounds(self):
        ok, _ = builder._validate_spec({"name": "X", "agg": "topk_share", "value_column": "value",
                                        "group_by": "agent", "k": 0.5, "compare": "level", "unit": "ratio"},
                                       {"agent", "value"})
        assert ok
        ok, msg = builder._validate_spec({"name": "X", "agg": "topk_share", "value_column": "value",
                                          "group_by": "agent", "k": 1.5, "compare": "level", "unit": "ratio"},
                                         {"agent", "value"})
...
class TestInputRef:
    def test_input_ref_composes_measure(self):
        specs = [
            {"name": "Count", "agg": "count", "value_column": None, "condition": None,
             "numerator": None, "denominator": None, "group_by": None, "k": None,
             "compare": "level", "unit": "count"},
...
class TestEngineVersion:
    ...
    def test_shell_does_not_resume_when_engine_changed(self, monkeypatch):
        ...
        def fake_compute(pri, df, schema_str, existing=None, on_progress=None):
            ...
            return {"generated_at": "t", "data_fingerprint": "d",
                    "engine_version": builder.COMPUTE_ENGINE_VERSION,
                    "period_definition": "p", "priorities": {"Growth": {"values": {}}}}
...
class TestVerifyTier:
    ...
    def test_verify_layers_flags_recompute_mismatch(self):
        values = {"Growth": {"status": "computed", "value": 99.0, "unit": "count",
                             "spec": _level_count_spec()}}
        updated, summary = builder._verify_layers(values, _sample_df(), _period())
    ...
    def test_verify_layers_flags_negative_level_count(self):
        values = {"Growth": {"status": "computed", "value": -1.0, "unit": "count",
                             "spec": _level_count_spec()}}
    ...
    def test_verify_priority_values_llm_unsets(self, monkeypatch):
        values = {
            "Growth": {"status": "computed", "value": 2.0, "unit": "count", "verified": True,
                       "measurement": "current count vs prior count", "spec": _level_count_spec()},
            "Deal Count": {"status": "computed", "value": 2.0, "unit": "count", "verified": True,
                           "measurement": "count of rows", "spec": _level_count_spec()},
        }
        monkeypatch.setattr(builder.llm, "ask_json",
                            lambda *a, **k: {"Growth": {"ok": False, "note": "wrong basis"},
                                             "Deal Count": {"ok": True, "note": "fine"}})
    ...
    def test_shell_verify_persists_verdicts(self, monkeypatch):
        ...
        values = {
            "Growth": {"value": 2.0, "unit": "count", "status": "computed", "verified": True,
                       "measurement": "current count vs prior count", "basis": "current 2 vs prior 2",
                       "spec": _level_count_spec()},
            "Deal Count": {"value": 2.0, "unit": "count", "status": "computed", "verified": True,
                           "measurement": "count of rows in current period", "basis": "current 2",
                           "spec": _level_count_spec()},
        }
        pv = {
            "engine_version": builder.COMPUTE_ENGINE_VERSION,
