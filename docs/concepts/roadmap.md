...
**Owner-confirmed metric policy (2026-08-13, from the scorecard roadmap doc):** two hard rules for authored metrics, to be enforced in the O2 validator + blueprint candidate gate — (1) **no single-dimension-member KPIs**: a `measurement` that references one member of a categorical column (e.g. "Express Air Ship Mode Volume Share", "Corporate Segment Profit Share Change") is excluded; define metrics across the whole dimension, per-member cells belong to the scorecard's columns, never in a metric definition; (2) **no analyst-only statistical indices** as KPIs/operational metrics (concentration / Herfindahl / Gini / leanness ratios like "Profit Concentration Index Change") — those are on-demand deep-dive tools, not scorecard line items. Enforceable deterministically via keyword gates on the metric name/measurement, alongside the existing SCALAR/COMPUTABLE tests.
